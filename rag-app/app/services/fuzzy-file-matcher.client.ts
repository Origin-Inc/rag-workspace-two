...
export class FuzzyFileMatcherClient {
  ...
  private static readonly SEMANTIC_MAPPINGS: Record<string, string[]> = {
    ...
    'movies': ['title', 'rating', 'genre', 'release', 'director', 'actor', 'film', 'cinema', 'vote'],
    ...
    'car': ['vehicle', 'auto', 'automobile', 'model', 'manufacturer', 'engine', 'mileage'],
    'economic': ['gdp', 'inflation', 'economy', 'indicator', 'growth', 'rate', 'value'],
    'health': ['fertility', 'disease', 'age', 'fever', 'alcohol', 'smoking', 'diagnosis'],
  };
  ...
  private static calculateFileMatch(
    ...
    // 2. Fuzzy match (simplified for client-side)
    const fuzzyScore = this.calculateSimpleFuzzyMatch(query, file, matchedTokens);
  ...
  /**
   * Simplified fuzzy matching for client-side performance
   */
  private static calculateSimpleFuzzyMatch(
    query: string,
    file: DataFile,
    matchedTokens: string[]
  ): number {
  ...
  /**
   * Calculate semantic match based on column names
   */
    ...
    for (const [keyword, threshold] of Object.entries(this.TEMPORAL_KEYWORDS)) {
      if (queryLower.includes(keyword)) {
        if (keyword === 'latest' || keyword === 'newest') {
          ...
          return Math.min(1, fileAge / (30 * 24 * 60 * 60 * 1000));
  ...
  static generateDisambiguationMessage(matches: FileMatchResult[]): string {
    ...
    const options = topMatches
      .map((m, i) => `${i + 1}. ${m.file.filename} (${Math.round(m.confidence * 100)}% match - ${m.reason})`)
      .join('\n');
    
    return `Multiple files matched your query:\n${options}\n\nUsing: ${topMatches[0].file.filename}`;
  }
}