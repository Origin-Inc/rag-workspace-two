...
-- Add GIN indexes for JSONB columns (only for existing columns)
...
CREATE INDEX IF NOT EXISTS idx_pages_content_gin ON public.pages USING gin(content);
...
BEGIN
    ...
    FOR perm_id IN (
        SELECT id FROM public.permissions 
        WHERE resource IN ('pages', 'documents', 'rag')
...
BEGIN
    ...
    ELSE
        RAISE NOTICE 'Embedding columns are configured.';
    END IF;
